...
def lanzar():
    ...
    for i in range (0,80):
        ...
        # 1 es humano y 0 es nohumano
        index = random.randint(0,1)

        tipo = Tipo_de_superheroe.from_index(index)
    ...
    print("Cada jugadore dispone de " + str(monedas) + " con las que poder comprar un maximo de " + str(escenario.get_n_miembros()) + " superheroes.")
    print(min(list_costes))

    for n in range(0,2):
        while len(sup_jug) < escenario.get_n_miembros() and monedas[n] >= min(list_costes):
